...
Require Export CertiGraph.graph.BinGraph.
...
Class pPointwiseGraph_Graph_Bin: Type := {
  addr: Type;
  null: addr;
...
Class sPointwiseGraph_Graph_Bin {pSGG_Bin: pPointwiseGraph_Graph_Bin} (DV DE: Type): Type := {
  pred: Type;
...
Class BinMaFin (g: PreGraph addr (addr * LR)) := {
  bin: BinGraph g (fun x => (x, L)) (fun x => (x, R));
...
Definition Graph := (GeneralGraph addr (addr * LR) DV DE DG (fun g => BinMaFin (pg_lg g))).
Definition Graph' := (GeneralGraph addr (addr * LR) DV DE DG (fun g => BinMaFin' (pg_lg g))).
...
Definition empty_BinGraph: BinGraph (empty_pregraph (fun e => fst e) (fun e => null)) (fun x => (x, L)) (fun x => (x, R)).
  constructor.
  + intros ? [].
  + intros ? ? [].
Defined.
...
Definition empty_sound: BinMaFin (empty_pregraph (fun e => fst e) (fun e => null)) :=
  Build_BinMaFin _ empty_BinGraph empty_MathGraph empty_FiniteGraph.
...
Definition is_BinMaFin (g: LGraph): Prop := exists X: BinMaFin (pg_lg g), True.

Definition is_BinMaFin' (g: LGraph): Prop := exists X: BinMaFin' (pg_lg g), True.

Definition is_guarded_BinMaFin (PV: addr -> Prop) (PE: addr * LR -> Prop) (g: LGraph): Prop := is_BinMaFin (gpredicate_sub_labeledgraph PV PE g).
...
Proof.
  intros.
  destruct lr.
  + apply (@left_sound _ _ _ _ _ _ g (binGraph _) x); auto.
  + apply (@right_sound _ _ _ _ _ _ g (binGraph _) x); auto.
Qed.
...
Proof.
  ...
  + destruct (@valid_graph _ _ _ _ g _ (maGraph _) (x, L) H) as [? _].
    pose proof (@only_two_edges _ _ _ _ g _ _ (binGraph _) _ (x, L) H0).
    simpl in H1.
    destruct H1 as [? _].
  ...
  + destruct (@valid_graph _ _ _ _ g _ (maGraph _) (x, R) H) as [? _].
    pose proof (@only_two_edges _ _ _ _ g _ _ (binGraph _) _ (x, R) H0).
    simpl in H1.
    destruct H1 as [? _].
    specialize (H1 (conj eq_refl H)).
    destruct H1; inversion H1.
...
Proof.
  ...
  refine (Build_BinMaFin _ (gen_dst_preserve_bin G (x, L) null _)
...
Proof.
  ...
  assert (weak_valid G null) by (left; reflexivity).
  refine (Build_BinMaFin _ (gen_dst_preserve_bin G (x, R) null _)
...
Proof.
  ...
  apply H3.
  + eapply left_valid in H; [| apply binGraph].
    auto.
  + eapply left_valid in H4; [| apply binGraph].
    auto.
Qed.
...
Proof.
  ...
  assert (vvalid g2 x) by (clear - H H0; firstorder).
  apply H3.
  + eapply right_valid in H; [| apply binGraph].
    auto.
  + eapply right_valid in H4; [| apply binGraph].
    auto.
Qed.
...
Proof.
  ...
  + intros.
    destruct X as [?H ?H ?H].
    apply (bin_graph_si _ _ (proj1 H)) in H0.
...
Lemma Graph_is_BinMaFin: forall (g: Graph), is_BinMaFin g.
Proof.
  intros.
  destruct g.
...
Lemma Graph'_is_BinMaFin': forall (g: Graph'), is_BinMaFin' g.
Proof.
  intros.
  destruct g.
  exists sound_gg; auto.
...
Proof.
  ...
  apply (bin_graph_si _ _ H) in H0.
...
Lemma is_guarded_BinMaFin_labeledgraph_add_edge: forall (g: LGraph) PV PE PE' e s d data_e,
  ~ evalid g e ->
  Same_set PE' (Intersection _ PE (fun e0 => e0 <> e)) ->
  is_guarded_BinMaFin PV PE g ->
  is_guarded_BinMaFin PV PE' (labeledgraph_add_edge g e s d data_e).
Proof.
  unfold is_guarded_BinMaFin.
  intros.
  eapply is_BinMaFin_si; [| eassumption].
  simpl.
...
Lemma is_guarded_BinMaFin'_labeledgraph_add_edge: forall (g: LGraph) PV PE PE' e s d data_e,
  ~ evalid g e ->
  Same_set PE' (Intersection _ PE (fun e0 => e0 <> e)) ->
  is_guarded_BinMaFin' PV PE g ->
  is_guarded_BinMaFin' PV PE' (labeledgraph_add_edge g e s d data_e).
...
Context {sSGG_Bin: sPointwiseGraph_Graph_Bin DV DE}.
...
Lemma is_guarded_BinMaFin_si: forall (g1 g2: LGraph),
*)

Lemma is_BinMaFin_LGraph_Graph: forall (g: LGraph) (P: LGraph -> pred),
  is_BinMaFin g ->
  P g |-- EX g: Graph, P g.
Proof.
  intros.
  destruct H as [X _].
  apply (exp_right (Build_GeneralGraph _ _ _ BinMaFin g X)).
  simpl.
  auto.
Qed.
...
Lemma va_labeledgraph_add_edge_eq: forall (g: LGraph) es e s d data,
  ~ evalid g e ->
  is_guarded_BinMaFin (fun x => s <> x) (fun e => ~ In e es) g ->
...
Proof.
  ...
  + unfold Included, Ensembles.In.
    intros x0 ?.
    destruct H0 as [X _].
    pose (g0 := Build_GeneralGraph _ _ _ (fun g => BinMaFin (pg_lg g)) _ X: Graph).
...
*********************************************************)
  ...
  Notation graph x g := (@reachable_vertices_at _ _ _ _ _ _ _ _ (_) _ (@SGP pSGG_Bin DV DE sSGG_Bin) _ x g).

  Lemma bin_graph_unfold: forall (g: Graph) x d l r,
  ...
  Lemma bin_graph_precise_left: forall (g: Graph) x l,
  ...
  Lemma bin_graph_precise_right: forall (g: Graph) x r,
