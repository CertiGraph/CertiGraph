...
Section GList_UnionFind.
  ...
  Proof.
    ...
    - destruct H5 as [? [l1 [? ?]]]. subst l. subst v. simpl. destruct H as [l2 ?]. destruct l2 as [? l2]. assert (a = x) by (destruct H as [[? _] ?]; simpl in H; auto).
      ...
      clear -H8. rewrite length_app in *. simpl in *. intuition auto with *.
  Qed.
  ...
  Proof.
    ...
    - destruct H5 as [? [l1 [? ?]]]. subst l. clear H4. subst v. simpl. destruct H7 as [[_ ?] [? _]]. specialize (H1 _ Hv _ H5 H4). simpl in H1. rewrite length_app. simpl length.
  ...
  Proof.
    ...
    - destruct H5 as [? [l1 [? ?]]]. subst l. clear H4. subst v. destruct (equiv_dec root root). 2: compute in c; exfalso; apply c; auto. simpl. destruct H7 as [[_ ?] [? _]].
      specialize (H1 _ Hv _ H5 H4). simpl in H1. rewrite length_app. simpl. clear -H H1. unfold UFGraph_LGraph in *. assert (vlabel (lg_gg g) x = vlabel g root) by intuition. intuition auto with *.
  Qed.
