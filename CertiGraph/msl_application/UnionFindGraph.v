...
Section UnionFindGraph.
  ...
  Definition UFGraph := (GeneralGraph V E DV DE DG (fun g => LiMaFin (pg_lg g))).
  ...
  *)
  ...
  Lemma valid_parent: forall (g: UFGraph) v n p, vvalid g v -> vgamma g v = (n, p) -> vvalid g p.
  Proof.
  ...
  Lemma parent_loop: forall (g: UFGraph) v n y, vgamma g v = (n, v) -> reachable g v y -> v = y.
  Proof.
  ...
  Lemma gamma_parent_reachable_included: forall (g: UFGraph) x r pa, vvalid g x -> vgamma g x = (r, pa) -> Included (reachable g pa) (reachable g x).
  Proof.
  ...
  Lemma Prop_join_reachable_parent: forall (g: UFGraph) x r pa,
  ...
  Definition Graph_gen_redirect_parent (g: UFGraph) (x: V) (pa: V) (H: weak_valid g pa) (Hv: vvalid g x) (Hn: ~ reachable g pa x): UFGraph.
  Proof.
  ...
  Lemma Graph_reachable_dec: forall (G: UFGraph) x, Decidable (vvalid G x) -> forall y, Decidable (reachable G x y).
  Proof.
  ...
  Definition Graph_vgen (G: UFGraph) (x: V) (d: DV) : UFGraph := generalgraph_vgen G x d (sound_gg G).

  Lemma uf_root_vgamma: forall (g: UFGraph) v n, vvalid g v -> vgamma g v = (n, v) -> uf_root g v v.
  Proof. intros. split; [apply reachable_refl | intros; apply (parent_loop g _ n)]; auto. Qed.

  Lemma vgamma_not_dst: forall (g: UFGraph) x r pa, vgamma g x = (r, pa) -> pa <> x -> dst g (out_edge x) = pa.
  Proof. intros. unfold vgamma in H. destruct (projT2 isNullDec (dst g (out_edge x))); inversion H; [exfalso |]; auto. Qed.

  Lemma vgamma_not_edge: forall (g: UFGraph) x r pa, vvalid g x -> vgamma g x = (r, pa) -> pa <> x -> g |= x ~> pa.
  Proof.
  ...
  Lemma vgamma_not_reachable': forall (g: UFGraph) x r pa y, vvalid g x -> vgamma g x = (r, pa) -> pa <> x -> reachable g pa y -> ~ reachable g y x.
  ...
  Proof.
    ...
    - assert (vvalid g2 x) by (rewrite <- H3; auto). specialize (H5 x H H6). unfold UFGraph_LGraph. rewrite H5. auto.
    - assert (evalid (predicate_partialgraph (lg_gg g1) (fun n : V => ~ reachable (lg_gg g1) pa n)) (out_edge x)). {
        simpl. hnf. pose proof (vvalid_src_evalid _ (liGraph g1) _ H). unfold UFGraph_LGraph in H6. destruct H6. rewrite H6. split; auto.
      } destruct H2 as [_ [? [_ ?]]]. pose proof H6. rewrite H2 in H8. clear H2. specialize (H7 _ H6 H8). simpl in H7. unfold UFGraph_LGraph. rewrite <- H7.
      destruct (projT2 isNullDec (dst (lg_gg g1) (out_edge x))); [exfalso |]; auto.
  Qed.

  Lemma the_same_root_union: forall (g g1 g2: UFGraph) x y root,
  ...
  Lemma uf_equiv_root_the_same: forall (g1 g2: UFGraph) x root, uf_equiv g1 g2 -> uf_root g1 x root <-> uf_root g2 x root.
  Proof. intros. pose proof (@uf_equiv_the_same_root _ _ _ _ _ _ _ _ out_edge isNullDec fml g1 g2 x root H). apply H0. Qed.

  Lemma uf_equiv_not_reachable: forall (g1 g2: UFGraph) x r pa root,
  ...
  Lemma graph_gen_redirect_parent_equiv': forall (g1 g2: UFGraph) x root,
  ...
  Lemma graph_gen_redirect_parent_equiv: forall (g1 g2: UFGraph) x r pa root (Hv: vvalid g2 x) (Hn: ~ reachable g2 root x),
  ...
  Lemma graph_gen_redirect_parent_findS: forall (g1 g2: UFGraph) x r1 r2 pa pa' (H: weak_valid g2 pa') (Hv: vvalid g2 x) (Hn: ~ reachable g2 pa' x),
    ...
    - assert (forall v, ~ reachable (lg_gg g1) x v -> ~ reachable (lg_gg g1) pa v). {
      ...
      + split; intros; destruct H13.
        * pose proof (H8 _ H14). pose proof (conj H13 H15). rewrite H10 in H16. destruct H16. rewrite H9 in H17. destruct H17. 1: split; auto. symmetry in H17.
          ...
          destruct H18 as [? _]. unfold UFGraph_LGraph in H18. rewrite H18 in H14. exfalso. apply H14. apply reachable_refl. auto.
        * pose proof (H8 _ H14). pose proof (conj H13 H15). rewrite <- H10 in H16. destruct H16. rewrite H9 in H17. destruct H17. 1: split; auto. symmetry in H17.
      ...
      + unfold updateEdgeFunc. destruct (equiv_dec (out_edge x) e).
        * unfold Equivalence.equiv in e0. subst e. pose proof (vvalid_src_evalid _ (liGraph g1) _ H0).
          destruct H15. unfold UFGraph_LGraph in H15. destruct H13. exfalso. apply H17. rewrite H15. apply reachable_refl; auto.
        * destruct H13, H14. apply H8 in H15. apply H8 in H16. apply H12; auto.
    - apply (graph_gen_redirect_parent_equiv _ _ _ r1 pa); auto.
    - apply H7.
  Qed.

  Lemma diff_root_union_1: forall (g g1 g2: UFGraph) x y x_root y_root,
  ...
  Lemma diff_root_union_2: forall (g g1 g2: UFGraph) x y x_root y_root,
  ...
  Lemma graph_gen_redirect_parent_vgamma: forall (g1 g2: UFGraph) x r pa y (H: weak_valid g1 x) (Hv: vvalid g1 y) (Hn: ~ reachable g1 x y),
