...
Definition path_correct (g: @DijkGG size inf) (prev dist: list V) src dst p : Prop  :=
  valid_path g p /\
...
Definition inv_unpopped (g : @DijkGG size inf) src (popped prev dist: list V) (dst: V) :=
  ~ In dst popped ->
  Znth dst dist < inf ->
  dst = src \/
  (dst <> src /\
...
Definition inv_unpopped_weak (g : @DijkGG size inf) (src: V) (popped prev dist : list V) (dst u : V) :=
  ~ In dst popped ->
  Znth dst dist < inf ->
  dst = src \/
  dst <> src /\
