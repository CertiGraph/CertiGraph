...
//I decided to keep the struct, it's good to figure out the tarray tstruct problem anyway
struct edge {
    int u;
    int v;
    int weight;
...
struct graph * init_graph_sample() {
    ...
    */
    ...
    struct edge *edge_list = mallocN(6*sizeof(struct edge));
...
struct graph * init_mst(int V) {
    ...
    struct edge * edge_list = mallocN(V*sizeof(struct edge ));
