...
\\\hspace{1em}\uncover<18->{\color{OliveGreen}mutator's max \texttt{alloc}?}
...
\\\uncover<2->{\hspace{7em} pointer?} 
\uncover<3->{\hspace{1em} in \texttt{from} space?}
\uncover<4->{\hspace{1em} already forwarded?}
\\\uncover<5->{\hspace{3em}and \alert{versatile}}
\\\uncover<6->{\hspace{7em}called on root set}
\uncover<7->{\hspace{1em}called on heap}
...
\begin{tikzpicture}[heap/.style={rectangle split, rectangle split parts=#1, draw,
        minimum width=20pt}, node distance=1cm, ->/.style={-Stealth, thick}]
\node [heap=5, rectangle split part fill={white, mypink, myred, mypink, myred}]
    (gen 0) {\nodepart{three}\ws{b}
            \nodepart{five}\ws{a}};
\node [heap=10, right = of gen 0.south east, anchor=south west,
      rectangle split part fill={white}]
        (gen 1) {};
\draw [->] (gen 0.two west)..controls ++(-0.5, -0.5).. (gen 0.four split west);

\tikzset{shift={(4,0)}}

\node {\Huge $\triangleright$};

\tikzset{shift={(2,0)}}

\node [heap=5, rectangle split part fill={white, mypink, myred, mypink, OliveGreen}]
    (gen 0) {\nodepart{three}\ws{b}
            \nodepart{five}\ws{a}};
\node [heap=10, right = of gen 0.south east, anchor=south west,
      rectangle split part fill={white, white, white, white, white, white, white, white, mypink, myred}]
        (gen 1) {\nodepart{ten}\ws{a'}};
\draw [->] (gen 0.two west)..controls ++(-0.5, -0.5).. (gen 0.four split west);
\draw [->] (gen 0.four east) -- (gen 1.nine split west);
...
\uncover<3>{\node at (3.25,-2) {$\gamma_{\alpha} \triangleright \gamma_{\omega}$};}
