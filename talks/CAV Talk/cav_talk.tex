...
Class SoundDijk size inf g := {
  ...
  ifr: $\alert<3>{\texttt{(MAX/size) * (size-1)}}$ $<$ inf;
  sz1: $\alert<4>{\texttt{size = 1}} \rightarrow \forall e, \texttt{ evalid g e } \rightarrow \texttt{ elabel g e } < \texttt{inf}$
}.
\end{lstlisting}
...
\end{tikzpicture}
 ...
 \uncover<4->{\hspace{1.8em}\texttt{sz1: }Special bounds for degenerate case \texttt{inf}}
