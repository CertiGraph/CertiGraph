...
\tikz\draw grid(5,5)foreach[count=~]\l in
{  , A, B, C, D, E,
 A, $\infty$, $\alert<3>{3}$, $\infty$, $\infty$, $\infty$,
...
\vspace{0.9em}
\uncover<4->{Requirement 1: graph, not multigraph}

\uncover<5->{Requirement 2: labels representable}

\uncover<6->{Requirement 3: $\exists \infty.$} \uncover<6->{$\infty$ representable} \uncover<7->{and no bona-fide edge has cost $\infty$}
...
\end{minipage} \uncover<2->{\begin{minipage}{0.45\textwidth}
{\centering
\begin{adjustbox}{scale=0.8}
\tikz\draw grid(5,5)foreach[count=~]\l in
{  , A, B, C, D, E,
...
\uncover<3->{\centering \textbf{Contribution 2: integrate undirected graphs into CertiGraph}}

\vspace{0.9em}

\quad \uncover<4->{Build lightweight undirected definitions} \\

\quad \uncover<5->{Prove connections to existing directed definitions} \\

\quad \uncover<6->{Grow undirected infrastructure}
...
\begin{lstlisting}
Class SoundDijk size inf g := {
  sadjmat: $\alert<2>{\texttt{SoundAdjMat size inf g}}$;
  efr: $\forall$e, evalid g e $\rightarrow$
           $\alert<3>{0~\le~\texttt{elabel g e}~\le~\texttt{(MAX/size)}}$;
  ifr: $\alert<4>{\texttt{(MAX/size) * (size-1)}}$ $<$ inf;
  sz1: $\alert<5>{\texttt{size = 1}} \rightarrow \forall e, \texttt{ evalid g e } \rightarrow \texttt{ elabel g e } < \texttt{inf}$
}.
\end{lstlisting}
...
[inf/.style={circle, draw=myred, thick, inner sep=0pt, minimum size=4pt},
 ->/.style={thick, arrows={-Stealth}}]
  ...
  \node at (1.5,-0.2) {\alert<4>5};
  \node at (4.9,-0.2) {\alert<4>5};
  \node at (6,-0.6) {\alert<3>6};
...
\end{tikzpicture}
 ...
 \uncover<2->{\hspace{1.8em}\texttt{sadjmat: }\texttt{SoundDijk} is an adjacency matrix} \\
 \uncover<3->{\hspace{1.8em}\texttt{efr: ~~~~}Leave room for probing link} \\
 \uncover<4->{\hspace{1.8em}\texttt{ifr: ~~~~}Bona-fide costs must dodge \texttt{inf}} \\
 \uncover<5->{\hspace{1.8em}\texttt{sz1: ~~~~}Special bounds for degenerate case for \texttt{inf}}
...
\uncover<2->{\textbf{Contribution 5: precise edge bounds to avoid overflow}}

\bigskip

\uncover<3->{\textbf{Contribution 6a: three adjacency matrix representations}}

\uncover<4->{(code bases differ by less than 1\%)}
...
%\begin{frame}{Binary heap with \texttt{decrease-key}}

%To get good performance, Dijkstra requires a priority queue that supports \texttt{decrease-key}.

\bigskip

\uncover<5->{\textbf{Contribution 7: certified binary heap}}
\uncover<6->{\textbf{with \texttt{decrease-key}}}

\uncover<7->{(several subtle C over/underflows discovered)}
\end{frame}
...
MST-PRIM(G,w,r):
 for each u in G.V
  /* elided: set up PQ,
     key, parent */
 $\alert<2>{\texttt{r.key = 0}}$
 while PQ $\neq$ $\emptyset$
  $\texttt{u = EXTRACT-MIN(PQ)}$
...
\node at (-0.5, 0.3) {\small B ($\infty$)};
\node at (3.45, 0.3) {\small C ($\infty$)};
\node at (-0.1, -3.5) {\small D ($\infty$)};
\node at (3.2, -3.5) {\small E ($\infty$)};
}

\uncover<3>{
\node[inv] (A) at (1.5,1.5) {};
\node[inv] (B) at (0,0) {};
\node[inv] (C) [right = 3 of B] {};
\node[inv] (D) [below = 3 of B] {};
\node[inv] (E) [below = 3 of C] {};

\node at (-0.4, 0.3) {\small B (3)};
\node at (3.2, 0.3) {\small C (4)};
\node at (-0.1, -3.5) {\small D (1)};
\node at (3.2, -3.5) {\small E (1)};
}
...
\uncover<4->{D can now be extracted at cost $\infty$... (!) \\ }
...
\uncover<1->{\textbf{Contribution 8: machine-certified ``real C'' Prim}}

\bigskip

\uncover<2->{\textbf{Contribution 9: more general specification}}

\bigskip

\uncover<3->{\textbf{Contribution 6b: three adjacency matrix representations}}

\uncover<4->{(code bases differ by less than 1\%)}

\bigskip

\uncover<5->{\textbf{Contribution 10: novel Prim variant without root}}
