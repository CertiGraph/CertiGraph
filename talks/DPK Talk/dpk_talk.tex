...
The first, in memory:
\begin{align*}
\uncover<5>{\m{arraddr} (\m{ptr}, \m{i},\texttt{size}) \defeq~&
  \m{ptr} + (\m{i} \times \hide{\texttt{sizeof}(\texttt{int}) \times} \texttt{size}) \\ }
\uncover<4->{\p{rowrep}(\gamma, \m{i}, \m{ptr}) \defeq~& \texttt{let }\m{row} \texttt{ := }\mathsf{graph2mat}(\gamma)[\m{i}] \texttt{ in } \\ 
...
\begin{tikzpicture}[on grid,
  ...
  \hspace{6.7em}\LARGE popped area + one hop}; }  
