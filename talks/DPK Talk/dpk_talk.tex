...
\texttt{MAX} = 15, \texttt{size} = 3, so every edge-label $\le 7$.}
...
\uncover<10>{so an edge-label is, at most, \alert{$\lfloor\texttt{MAX/(size-1)}\rfloor$}}
...
Class SoundDijk size inf g := {
  ...
  efr: $\forall$e, evalid g e $\rightarrow$
           0 $\le$ elabel g e $\le$ (MAX/size);
  ifr: (MAX/size) * size $\le$ inf 
}.
\end{lstlisting}
...
\uncover<7->{\hspace{-.35em}meaning D is the root of a new tree!}
\end{minipage} \begin{minipage}{0.35\textwidth}
\uncover<2->{
{\centering
\begin{adjustbox}{scale=0.9}
...
\uncover<1->{
Mathematical, spatial support for adjacency matrices \\
Mathematical, spatial support for edge lists \\
Mathematical support for undirected graphs \\ }
\bigskip
\uncover<2->{
Dijkstra's algorithm: nontrivial overflow \\ }
\uncover<3->{
Prim's algorithm: nontrivial improvement in spec \\ }
\uncover<4->{
Kruskal's algorithm: library handles directed $+$ undirected \\ }
\bigskip
\uncover<5->{
