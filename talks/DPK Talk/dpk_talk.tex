...
\texttt{vvalid}&\defeq\lambda v.~0 \le v < \texttt{size}, \\ 
...
\hspace{1em} small variations (1\% loc) between spatial representations
...
\begin{lstlisting}
  ...
  if (cost < inf && dist[i] > dist[u] + cost) {
...
$\texttt{cycle}^{u}$, $\texttt{forest}^{u}$, $\texttt{spanning}^{u}$, and
...
\bigskip \pause
\texttt{u} can now be extracted at cost \texttt{INF} \\ 
\bigskip \pause
