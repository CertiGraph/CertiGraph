...
\uncover<10>{so an edge-label is, at most, \alert{$\lfloor\texttt{MAX/size}\rfloor$}}
...
Class SoundDijk size inf g := {
  ...
  efr: $\forall$e, evalid g e $\rightarrow$
           0 $\le$ elabel g e $\le$ $\alert<1>{\texttt{(MAX/size)}}$;
  ifr: $\alert<2>{\texttt{(MAX/(size-1)) * size}}$ $<$ inf 
}.
\end{lstlisting}
...
[inf/.style={circle, draw=myred, thick, inner sep=0pt, minimum size=4pt},
 ->/.style={thick, arrows={-Stealth}}]
  ...
  \node at (1.5,-0.2) {\alert<2>5};
  \node at (4.9,-0.2) {\alert<2>5};
  \node at (6,-0.6) {\alert<1>6};
...
\end{tikzpicture}
 ...
 \uncover<1->{\hspace{1.8em}\texttt{efr: }Leave room for probing link} \\
 \uncover<2>{\hspace{1.8em}\texttt{ifr: }Bona-fide costs must dodge \texttt{inf}} 
