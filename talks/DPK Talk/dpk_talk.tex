...
\uncover<2->{so an edge-label is, at most, $\lfloor\texttt{MAX/(size-1)}\rfloor$}

\bigskip \uncover<3->{
\texttt{MAX} = 7, \texttt{size} = 3, so every edge-label $\le 3$.}
...
\node at (3.7, 0.3) {\small B (cost \ifty)};
\node at (6.8, 0.3) {\small C (cost \ifty)}; 
  \node[inf] (A) at (0,0) {};
  \node[inf] (B) [right = 3 of A] {};
  \node[inf] (C) [right = 3 of B] {};
  \draw [->, dashed, myred] (A) -- (B);
...
\uncover<8->{ 
\node at (0.3, 0.3) {\small A (cost 0)};
\node at (3.5, 0.3) {\small B (cost \alert{1})};
\node at (6.6, 0.3) {\small C (cost 6)}; 
...
\vspace{0.9em}
\uncover<2>{\centering Inadvertently created a fully-connected graph!}
...
The first, in memory:
\begin{align*}
\m{arraddr} (\m{ptr}, \m{i},\texttt{size}) \defeq~&
  \m{ptr} + (\m{i} \times \hide{\texttt{sizeof}(\texttt{int}) \times} \texttt{size}) \\
\p{arrrep}(\gamma, \m{i}, \m{ptr}) \defeq~& \texttt{let }\m{row} \texttt{ := }\mathsf{graph2mat}(\gamma)[\m{i}] \texttt{ in } \\
&\mathsf{array}(\mathit{arraddr}(\m{ptr},\m{i},\lvert\m{row}\rvert), \m{row}) \\
\vspace{1em}
\p{AdjMat}(\gamma, \m{gaddr}, \_) \defeq~& \underset{\m{v}\hspace{0.1em}\in \gamma}{\bigstar} \p{arrrep}(\gamma, \m{v}, \m{gaddr})
\end{align*}
}
...
{  , A, B, C, D, E,
 A, $\infty$, 3, $\infty$, $\infty$, $\infty$,
 B, $\alert{\times}$, $\infty$, 4, 6, $\infty$,
 C, $\alert{\times}$, $\alert{\times}$, $\infty$, 1, 2,
 D, $\alert{\times}$, $\alert{\times}$, $\alert{\times}$, $\infty$, 1, 
 E, $\alert{\times}$, $\alert{\times}$, $\alert{\times}$, $\alert{\times}$, $\infty$}
{({-.5+mod(~-1,6},{5.5-div(~-1,6})node{\l}};
\end{adjustbox}
...
MST-PRIM(G,w,r):
 ...
 r.key = 0
 while PQ $\neq$ $\emptyset$
  $\alert<6->{\texttt{u = EXTRACT-MIN(PQ)}}$ 
  for each v in G.Adj[u]
   /* potentially add v 
      and (u, v) to MST */
\end{lstlisting} 
...
\uncover<6->{D can now be extracted at cost \texttt{inf} \\ } 
\uncover<7->{meaning D is the root of a new tree!}
...
\uncover<2-4>{\node at (2.3,1.1) {9};}
 ...
 \uncover<4->{\draw [thick, myred] (C) -- (B);}
 \uncover<8>{\draw [thick, myred] (E) -- (D);}
