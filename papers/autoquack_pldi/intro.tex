...
\begin{itemize}
\item[\S\ref{sec:orientation}] We verify a graph marking algorithm in both VST using two new Floyd tactics, \li{localize} and \li{unlocalize}.  We introduce \emph{localization blocks} as a new notation for ramification.  We verify the same graph marking algorithm in H/S using automatic localization.  We discuss three additional VST-certified examples: marking a DAG, pruning a graph into a spanning tree (\emph{e.g.} for disposal), and making a structure-preserving copy of a graph.
