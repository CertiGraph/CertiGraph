...
As mentioned in \S\ref{sec:orientation},  \infrulestyle{Ramify} has a side condition that $(L_2 \wand G_2) \text{ ignores } \MV(c)$. However, when the local program $c$ contains local variable operation, this side condition is usually not true. One potential way to avoid this local variable issues is to use ``variables as resource''~\cite{bornat:var}.
However, neither VST nor HIP/SLEEK use variables as resource. Most other mechanized verification systems do not use variables as resource either ~\cite{Beckert:2007,DistefanoP08,bengtson:charge}.

In this section, we present \infrulestyle{Ramify-P}, a generalized version of \infrulestyle{Ramify}, to handle this local variables.
\vspace{-0.75ex}
...
With \infrulestyle{Ramify-P} and \infrulestyle{Solve Ramify-P} we can
prove the \infrulestyle{Frame} rule with its canonical side condition
as follows (this justifies our point in \S\ref{sec:localizations} that our new localization notation can also be used for frames):
\vspace{-1ex}
\[
...
{\m{precise}(L_1)}
\]
The side condition adds the requirement that local state $L_1$ is \emph{precise} in the usual sense in separation logic:
\[
\begin{array}{@{}l@{}l@{}}
