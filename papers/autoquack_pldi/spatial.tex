...
\subsection{Traditional fixpoints fail for \p{graph} predicates}\label{sec:fixpointfail}
...
Rather than trying to define \p{graph} as a recursive fixpoint, we will instead give it a flat structure.  As discussed in \S\ref{sec:related}, graphs in separation logic have been defined in similar ways before; what is new here is that we prove---with the amount of precision required to convince Coq---that we can still enjoy fold/unfold with our definition.  Our path starts with the iterated separating conjunction or ``big star'', defined as:
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%%%% Edit1
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
\end{eqnarray*}
The big star $\bigstar$ is first defined over a list. Then the definition extended to sets. Now, we are ready to give a good \p{graph} predicate:
\fi
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
\end{equation}
$\gamma$ is a GeneralGraph and ``$x |-> \gamma(x)$'' is a predicate that describes how single nodes fit in memory; in Figure~\ref{fig:markgraph} it was:
\[
...
%%%%%% Edit2
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%%%% Edit2: Qinxiang's proposal starts
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\iftrue
...
& ~~~ \text{where } ~ \underset{l_1,\dots,l_n}{\raisebox{-0.3ex}{\resizebox{0.75em}{!}{$\scon$}}\hspace{-2.45ex} \bigcup} \! \! P ~~ \defeq ~~ P(l_1) ** \ldots ** P(l_n) \notag
\end{eqnarray}
\fi
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%%%% Edit2: Qinxiang's proposal ends
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%%%% Edit2: Original version starts
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\iffalse
Our definition of \p{graph} is flat in the sense that there is no obvious way to follow the link structure recursively.  Happily, we can recover a general recursive fold/unfold if $x |-> \gamma(x)$ and the GeneralGraph have the necessary properties.  To state this lemma we need the iterated overlapping conjunction:
\[
\underset{l_1,\dots,l_n}{\raisebox{-0.3ex}{\resizebox{0.75em}{!}{$\scon$}}\hspace{-2.45ex} \bigcup} \! \! P ~~ \defeq ~~ P(l_1) ** \ldots ** P(l_n)
\]
...
\p{graph}(x,\gamma) ~~ <=> ~~ x |-> \gamma(x) ** \Big(\!\!\!\!\!\!\!\!\!\!\!\!\underset{n \in \p{neighbors}(\gamma,x)}{\raisebox{-0.3ex}{\resizebox{0.75em}{!}{$\scon$}} \hspace{-2.45ex} \bigcup}\!\!\!\!\!\!\!\!\!\!\!\! \p{graph}(\gamma,n) \Big)
\end{equation}
We get a full equivalence between a graph and its ``unfolded'' structure, regardless of how many neighbors $x$ has.
\fi
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%%%% Edit2: Original version ends
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

The proof of $<=$ direction requires care. The difficulty is that if two nodes $x |-> \gamma(x)$ and $x' |-> \gamma(x')$ are \emph{skewed}, \emph{i.e.} ``partially overlapping'' with some---but not all---of $x$'s memory cells shared with $x'$, then the $\bigstar$ on the left hand side cannot separate them.  To avoid skewing we require $x |-> \gamma(x)$ be \emph{alignable}.  A predicate $P$ is alignable when
\[
...
Above the Logic layer we have three towers, each three levels high.  The leftmost tower is about basic lemmas about Logic, $\bigstar$, and \p{graph}.  For example, in the $\bigstar$ Facts box we prove lemmas such as the following:
\[
\infrule{}
{A \cap B = \emptyset}
