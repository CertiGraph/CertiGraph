...
Along the way we discover---and show how to fix---a rather surprising error in Hobor and Villard's framework; generalize their setting
so that it can handle a substantially wider variety of data structures; 
...
\hide{
1. General purpose part
1.1. A lot of important theorems about graph are proved.
1.2. Iter_sepcon and pred_sepcon are defined. And related ramification rules are proved.
1.3. The most general graph-spatial-predicate vertices_at are defined (for all possible styles of graphs). Related ramification rules are proved. Graph and graphs are defined as special cases of vertices at.
...
2.2. pSpatialGraph_Graph_Bi defined in "spatial_graph_bi.v" is the premise type class for defining BiMaFin.
2.3. sSpatialGraph_Graph_Bi defined in "spatial_graph_bi.v" is the premise type class for defining how a BiMaFin graph is stored in memory.
2.4. VST instance of pSpatialGraph_Graph_Bi and sSpatialGraph_Graph_Bi are constructed in "spatial_graph_aligned_bi_VST.v" and "spatial_graph_unaligned_bi_VST.v".
2.5. Heap-Model-Direct instances are broken now. They are not hard to get fixed. Previous proofs are in "spatial_graph_HMD.v".
