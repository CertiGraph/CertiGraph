...
\node (SM) at (0, 0) {\small Step-Indexing Model};
\node (DM) [right=2 of SM] {\small Direct Model};
\node (CL) [above=0.8 of SM] {\small Core Logic};
\node (SL) [above=0.8 of DM] {\small Supplementary Logic};
\node (R) [above=0.8 of CL] {\small Ramification};
\node (SR) [above=0.8 of SL] {\small Suppl. Ramification};
\node (G) [above=0.8 of R] {\small Graphs};
\node (SG) [above=0.8 of SR]{\small Supplementary Graphs};
...
\draw [->] (R) to (G);
\draw [->] (R) to (SR);
\draw [->] (CL.west) to [bend left=40] (G.west);
\draw [->] (SL.east) to [bend right=40] (SG.east);
\end{tikzpicture}
\endpgfgraphicnamed
\vspace{1ex}
