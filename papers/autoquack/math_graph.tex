...
\node[realG] (LG) [below=1 of PG] {\small LabeledGraph};
\node[realG] (GG) [right=3 of LG] {\small GeneralGraph};
...
\node[propG] (BMF) [below=1 of SC] {\small BiMaFin};
\draw [double,->] (BMF) -- (SC) node [pos=0.5, left] {\small Instantialize};
\node[propG] (BG) [left=1 of BMF] {\small BiGraph};
\node[propG] (MG) [below=0.5 of GG] {\small MathGraph};
\node[propG] (FG) [below=0.5 of MG] {\small FiniteGraph};
\draw [->] (BG) to (BMF);
\draw [->] (MG) to (BMF);
\draw [->] (FG) to (BMF);
\end{tikzpicture}
