...
\subsection{Structure of the mathematical graph framework}
...
which a bare PreGraph can not inculde. {\color{magenta} For example,
many algorithms about graphs assume there are labels in vertices or
edges.}

For example, when we compute some properties of a graph, we always
hope the graph is finite. In the recursive defnition of a graph data
structure, a node contains many pointers to point to its
