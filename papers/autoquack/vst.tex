...
The decorated program on the left is actually representing the Hoare logic proof on the right side. When users prove a Hoare triple, VST enables them to feel as if they were write a decorated program from up to down, but the proof built in Coq is has a structure as an inference diagram.

Specifically, when the proof goal is $\triple{P_1}{c1;c2}{P_5}$, VST's user can apply some Hoare rule to get a triple for c1, e.g. $\triple{P_1}{c1}{P_2}$. VST system then applies sequence rule automatically and the proof goal left to user will be $\triple{P_2}{c2}{P_5}$. On user's view, he/she finishes the first three lines in his/her decorate program. But in Coq's underlying logic, VST system is build a proof tree from bottom to the top.
