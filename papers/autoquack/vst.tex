...
The decorated program on the left is actually representing the Hoare logic proof on the right side. By adding a new pattern, we call it localized and unlocalize, ramification proofs can also be presented in a decorated programs.
...
Apart from enabling mechanized program verification in Coq, VST establishes a connection between Hoare logic proofs and nice printed decorated programs. For example,

When users prove a Hoare triple, VST enables them to feel as if they were write a decorated program from up to down, but the proof built in Coq has a structure as an inference diagram.

Specifically, when the proof goal is $\triple{P_1}{c1;c2}{P_5}$, VST's user can apply some Hoare rule to get a triple for c1, e.g. $\triple{P_1}{c1}{P_2}$. VST system then applies sequence rule automatically and the proof goal left to user will be $\triple{P_2}{c2}{P_5}$. On user's view, his/her proof goal changes from $\triple{P_1}{c1;c2}{P_5}$ to $\triple{P_2}{c2}{P_5}$ and this means that he/she finishes the first three lines in his/her decorate program. But in Coq's underlying logic, VST system is build a proof tree from bottom to the top.

\subsection{Ramification in VST}

Our work of embedding ramification into VST system involves 3 parts. 1: Prove all ramification rules sound in Coq. 2: Establish a connection between ramification rule in decorated programs and in proof trees. 3: Write a Coq tactic system which implements this connection.
