...
\texttt{decrease\_key} operation, which can be implemented in $\bigO(\log n)$
...
(see line~\ref{code:update1}). It is thus possible to maintain a list of 
examined vertices, and, when looking for the next-cheapest unexamined
vertex, to simply scan \texttt{dist} for the cheapest vertex that has not
