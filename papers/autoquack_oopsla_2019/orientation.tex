...
This captures the essence of finding the root and doing path compression: after compressing your parent and finding its root, path-compress yourself by rerouting your own parent pointer to your (soon-to-be former) parent's root.  The existential in the goal is nontrivial
...
marking cyclic graphs: on the one hand, we get genuine separation
between the root and its children; on the other hand, we need to maintain acyclicity if
we modify the link structure.
We verify \li{copy}, a graph copying algorithm. Here the challenge is 
that the copied graph does not satisfy the original graph's soundness 
condition (see~\S\ref{subsec:graphplugins}) while it is being constructed, 
