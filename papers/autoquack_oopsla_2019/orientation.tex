...
The whole graph (the entire disjoint-set forest) is represented by
...
with some ``local'' pre- and postconditions $L_1$ and $L_2$ using an additional ramification
frame $R$.  
...
What is \textbf{not} specified explicitly is the ramification frame $R$, which must satisfy 
...
$R \triangleq L_2 --* G_2$, which makes the second entailment trivial, and reduces the problem
...
problem: the free variables of $R \triangleq L_2 --* G_2$ are \textbf{not} disjoint from 
...
Accordingly, our proof fails with the choice of $R \triangleq (L_2 --* G_2)$.
...
choose $R \triangleq L_2 --* G_2$ exactly written in lines~\ref{code:findafterxparent} 
