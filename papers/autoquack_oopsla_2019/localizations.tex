...
With careful engineering, all of the modified-variable dance above can be done fully automatically, and in a way that is completely hidden to end-users.  The only remaining proof
...
In addition to the proof of \li{find} shown above, we do a proof of \li{union}
in the same style.  We also do a \emph{second} version of union-find, this time using
arrays rather than \li{malloc}-allocated nodes.  The mathematical definitions,
...
We also have done a version of \li{mark} for DAGs (acyclic).  In general, using DAGS
