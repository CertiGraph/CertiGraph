Here we explain two important features of the \infrulestyle{Localize} rule that go beyond what \infrulestyle{Ramify} can do: its ability to link existentially-quantified predicates from the local postcondition $L_2$ to the global postcondition $G_2$, as well as how to handle program variables that are modified during a localization block.  Perhaps surprisingly, to provide a robust, lightweight, and fully-automatable solution to the latter, it is necessary to first understand and solve the former.
...
We take advantage of compositionality by using $\m{mark}(\gamma,x,\gamma') /| \ldots$ to specify both our ``spanning tree'' and {\color{blue}``graph copy''} algorithms in~\S\ref{sec:application}, which also mark nodes while carrying out their primary tasks.
