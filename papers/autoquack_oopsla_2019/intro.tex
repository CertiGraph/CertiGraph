...
\item[\S\ref{sec:localizations}] We show that \textsc{Localize} and \textsc{Frame} are co-derivable.  We illustrate a delicate technique to properly handle modified local variables.  We show a mark-graph
program that explores a graph in a fold/unfold style and discuss the utility of linked
existentials in postconditions.  
We give a sense of the breadth of algorithms we can verify, which include marking a DAG, an array-based
