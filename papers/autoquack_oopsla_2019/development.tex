...
Floyd is written in Ltac and Gallina and is designed to help users verify the full functional correctness of their programs.

~\cite{appel:programlogics}, especially those programs that can be compiled with the compiler~\cite{leroy:compcert}.

concrete programs.

The Verified Software Toolchain consists of a series of machine-checked modules written in Coq
to reason about (CompCert) C programs.  Floyd use

One of VST's modules,

.
 whose focus is reasoning about

The modules interlock so there are no ``gaps'' in the end-to-end certified results; accordingly all of the rules employed by Floyd have been proved sound with respect to the underlying semantics used by CompCert.
