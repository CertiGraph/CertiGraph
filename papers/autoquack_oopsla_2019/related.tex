...
Hobor and Villiard verified graph algorithms using the \infrulestyle{Ramify} rule.  
...
of \infrulestyle{Ramify} called \infrulestyle{RamifyAssign}, which could reason about the 
special case of a single assignment $\li{x=}f(\ldots)$, assuming the verifier can make 
the local program translation to $\li{x'=}f(\ldots)\li{; x=x'}$, where \li{x'} is fresh.  
...
\note[from inv-ack. massage to avoid plagiarism]{Charguéraud and Pottier,
...
others are manual. \note{Does he have a model like ours??}
