...
program behavior. \noted{Accordingly, an essential component of our
work is a library of formalized graph theory which is powerful and
expressive enough to verify realistic algorithms in a machine-checked
theorem prover.}
One of its strengths is its modularity, which
...
\input{kindsofgraphs.tex}
\input{somegraphdefns.tex}
...
\paragraph{PreGraph.}             
A PreGraph is a hextuple $(\mathcal{V}, \mathcal{E}, \mathtt{V}, \mathtt{E}, \mathtt{src}, \mathtt{dst})$, where $\mathcal{V}$ and $\mathcal{E}$ are the underlying
carrier types of vertices and edges, and $\mathtt{V}$ and $\mathtt{E}$, predicates over
$\mathcal{V}$ and $\mathcal{E}$ respectively, introduce the notion
of \emph{validity} in the graph. Finally, $\mathtt{src}$ and $\mathtt{dst}$ are functions
that map an edge to its source and destination. With an 
...
In set theory, one
element can belong to multiple sets, 
...
predicates $\mathtt{V}$ and $\mathtt{E}$ to classify vertices and edges as \emph{valid}
(in) or \emph{invalid} (out) \note{of a graph}. The equivalent of 
\note[depends on answer to earlier doubt]{superset/subset behavior} can be replicated by weakening or strengthening the two predicates.

\note{The other benefit is the ability to represent incomplete graphs, which is
convenient in verification.
Figure \ref{fig:pregraph} shows an incomplete graph, 
...
Coq definitions of the concepts marked with asterisks are 
shown in Figure \ref{fig:graphdefns}.
These general concepts, together with derived lemmas 
(around 750 in total) provide a
...
\note{add: updating a graph requires care with the dependent types. What is a d.t.}
