...
system. However, its more distinctive property is its modularity, 
...
subsets $VT$ and $ET$ respectively, allow for a notion of \emph{validity} in the 
graph. For instance, in Figure \ref{fig:pregraph}, valid vertices are in $V$ and 
invalid vertices are in $VE \smallsetminus V$ but, importantly, 
both kinds are legally part 
...
requirements of a legal PreGraph, not even a notion of how the four sets are related.
...
$\m{a} \mathrel{{\stackrel{\gamma~}{\leadsto^{1}}}} \m{b} \defeq a \in V(\gamma) /| 
b \in V(\gamma) /| \exists e.~ e \in E(\gamma) /| s (e,\gamma) = a /| 
d (e,\gamma) = b$. 
...
verifying real programs. For instance, in \ref{blah} we argued for a condition of 
congruence on $\gamma \smallsetminus (v \in \gamma \mid \m{x} 
\mathrel{{\stackrel{\gamma~}{\leadsto^{\star}}}} \m{v})$. 
Even if $\gamma$ were to satisfy some stronger notion of
``well-formed'', in the sense that valid vertices have only valid edges and 
vice versa, subtracting some vertices in this way may well make $\gamma$ ill-formed 
by leaving dangling edges pointing to recently-removed vertices. 
A PreGraph is more accommodating:
...
vertices, edges, and/or the graph as a whole. As shown in Figure~\ref{graphs}, a 
LabeledGraph inherits any lemmas proved about its associated PreGraph. In addition, we 
can define additional lemmas that use labels, \emph{e.g.} {add an integer label to 
vertices denoting \emph{rank} and state a lemma that \texttt{find} does not alter
any vertex's rank, or, more interestingly, add string labels to edges and reason 
about a trie.}
\fi
...
A GeneralGraph augments a LabeledGraph by adding a soundness condition, indicated in 
Figure~\ref{fig:graphs} by a dashed border. This soundness ``plugin'' makes 
GeneralGraphs truly versatile: it allows us to state algorithm-specific properties 
...
We use Coq's typeclass system to manage our plugins smoothly, benefiting from
the \emph{compositionality} of the system. If we have two properties, 
each with its associated proved lemmas, we can combine them, {\color{magenta}easily} 
prove the {\color{magenta}emergent} lemmas that result from the combination, and then 
treat the new combination as a new plugin. 
...
%\vspace{-1ex}
\item \p{BiGraph}: there are two outgoing edges per node. 
\item \p{LstGraph}: the graph behaves like a list; every
vertex has one out edge and there are no loops.
\end{itemize}
...
Now we can, for instance, compose \p{LstGraph}, \p{MathGraph}, and \p{FiniteGraph} 
together into a new plugin called \p{LiMaFin}, which is the 
actual soundness condition used to verify the program in Figure~\ref{fig:findsketch}.
In our verification of Mark~\ref{blah}, we use a similar soundness condition
\p{BiMaFin}, which uses \p{BiGraph} instead of \p{LstGraph}. 
Composing plugins in this way is much more elegant than creating custom plugins
for each new algorithm as it promotes a natural reuse of graph 
properties across algorithms.

{\color{blue} Maybe move this somewhere.} Coq also handles our notion of inherited 
lemmas seamlessly: as described earlier,
we use PreGraph properties such as reachability and selective subtraction on a 
\p{LiMaFin} GeneralGraph and trust Coq to insert the appropriate coercions. 
