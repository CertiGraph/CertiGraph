...
Even if $\gamma$ itself satisfied some stronger notion of
``well-formed'', in the sense that valid vertices have only valid edges and 
vice versa, subtracting some vertices may well make 
$\gamma \smallsetminus (v \in \gamma \mid \m{x} 
\mathrel{{\stackrel{\gamma~}{\leadsto^{\star}}}} \m{v})$ ill-formed 
...
\emph{e.g.} add an integer label, denoting \emph{rank} to 
vertices and prove a lemma that running \texttt{find} does not alter
any vertex's rank. 
...
we explain how we enjoyed {\color{magenta}modularity} in this regard as well.
...
we can combine them, 
prove the {\color{magenta}emergent} lemmas that result from the combination
{\color{magenta}with relative ease}, and then 
treat the combination as a new plugin. 
...
about these properties. However, this is still too general for 
most real programs. The next step is compose these plugins to 
arrive at a more specific set of restrictions that more closely 
models our particular graph. 
...
soundness condition used to verify Find in Figure~\ref{fig:findsketch}.
...
we saw, we still use
PreGraph properties such as reachability and selective subtraction 
on this GeneralGraph. Coq handles the appropriate coercions with 
remarkable elegance.
