...
Standing as we do on the shoulders of giants, we can see further and more clearly.  The invariants at each program point are natural despite only minor tidying from our machine-checked proof.  We also enjoy good separation between the spatial predicates and pure predicates.  All of this is despite verifying real C code, forcing us to prove grungy details such that the pointer comparison in line~\ref{code:ufif} is defined\footnote{\label{footnote:pointereq}Specifically, whenever \texttt{x} and \texttt{p} are both null; or when one of them is null and the other has offset between 0 and the size of the memory block into which it is pointing; or when if \texttt{x} and \texttt{p} are from the same memory block, then both of their offsets are between 0 and the size of that block; or when \texttt{x} and \texttt{p} are not in the same memory block and both have offsets between 0 and the size of their respective memory blocks \emph{minus one}.  In line~\ref{code:ufif}, this comes from the spatial graph predicate $\p{uf\_graph}(\gamma)$ and the pure facts $\texttt{x} \in V(\gamma)$ and {\color{magenta} $\gamma(\mathtt{x}) = (r,\texttt{pa})$}.}.
...
Most of the verification is straightforward: in fact, each individual line of code (\ref{code:findram1}, \ref{code:ufif}, \ref{code:ufreccall}, \ref{code:ufpathcompress}, and \ref{code:ufreturn}) is bracketed with exactly the invariants one would hope for to easily verify that line (ignoring the symbols $\searrow$ and $\swarrow$ for one paragraph).  In addition to improving human comprehensibility, this is useful because it means that the underlying verification engine (VST, in our case) can handle many grungy details (\emph{c.f.} footnote~\ref{footnote:pointereq}) for us if we provide some suitable lemmas.
