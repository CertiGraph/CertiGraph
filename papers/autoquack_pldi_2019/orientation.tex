...
As an initial demonstration of our techniques, we show the decorated code of the path-compressing \texttt{find} function from the classic disjoint-set data structure~\cite{CLRS} in Figure~\ref{fig:ufcode}.  At first, union-find appears rather trivial since nodes have only one outgoing pointer and the body of \texttt{find} has only about 5 lines of code.  In fact, however, the disjoint-set data structure is notoriously difficult to reason about: indeed, the first pen-and-paper verification in separation logic required 20 pages~\cite{neelthesis}.

Standing as we do on the shoulders of giants, we can see further and more clearly.  The invariants at each program point are natural despite only minor tidying from our machine-checked proof.  We also enjoy good separation between the spatial predicates ($\p{uf\_graph}(\gamma)$ and maps-to $|->$) and the pure predicates ($\texttt{x} \in V(\gamma)$, $\m{findS}(\gamma,\m{pa},\gamma')$, etc.).  All of this is despite our focus on verifying real C code, forcing us to prove grungy details such that the pointer comparison in line~\ref{code:ufif} is defined\footnote{Specifically, whenever \texttt{x} and \texttt{p} are both null; or when one of them is null and the other has offset between 0 and the size of the memory block into which it is pointing; or when if \texttt{x} and \texttt{p} are from the same memory block, then both of their offsets are between 0 and the size of that block; or when \texttt{x} and \texttt{p} are not in the same memory block and both have offsets between 0 and the size of their respective memory blocks \emph{minus one}.  {\color{blue} In line~\ref{code:ufif}, this fact is derived from the spatial graph predicate $\p{uf\_graph}(\gamma)$ together with the pure facts that $\texttt{x} \in V(\gamma)$ and {\color{magenta} $\gamma(\mathtt{x}) = (r,\texttt{pa})$}; we will see more about these definitions {\color{yellow} shortly}.}}.

The specification we verify has its precondition on line~\ref{code:ufpre}: we require a heap representation of the mathematical union-find graph $\gamma$ ($\p{uf\_graph(\gamma)$ represents the entire disjoint-set forest) and that \texttt{x} is a vertex in $\gamma$.  The postcondition of \texttt{find} is on line~\ref{code:ufpost}: the heap is representing a new union-find graph $\gamma''$, which is mathematically related to the original graph $\gamma$ via the pure relation $\m{findS}$, which describes the action of path compression starting from node \texttt{x}.  Moreover, the function returns (\texttt{ret}) the node \m{rt}, which is the root (ultimate parent) of \texttt{x}, as specified by the pure relation $\m{uf\_root}$ (by $\m{findS}$, we know that path compression has not changed the root of any node between $\gamma$ to $\gamma''$).
