...
\title{Verified C implementations of classic textbook graph algorithms}
...
% If there are more than two authors, 'et al.' is used.
	...
	\begin{abstract}
		\vspace{-1.2em}
		This paper provides proof-verified graph implementations of Dijkstra's, Prim's and Kruskal's graph algorithms.
		We prove functional correctness of our C implementations, and improve Wang et al\cite{DBLP:journals/pacmpl/WangCMH19}'s CertiGraph platform to reason about undirected graph properties and common spatial representations.
		
		In particular, we encounter and explain an overflow issue in Dijkstra’s algorithm. The precise bound in the relevant precondition is nontrivial: we show that the intuitive guess fails and provide a workable refinement. We also make an observation of Prim's algorithm that contradicts the common notion that the basic algorithm cannot work on disconnected graphs - our Prim's implementation is able to compute minimum spanning forests without finding components first.
