...
nonstandard \texttt{decrease\_key} 
...
\m{dist}, all the vertices visited by the path are also popped,
...
\begin{enumerate}
\item $\m{inv\_popped}$: $\m{dst}$ has been fully processed and
popped from the PQ.  For all popped vertices \emph{except for}~\m{u},
this is trivial from $\m{dijk\_correct}$. For~\m{u} itself, we reach the heart of Dijkstra's correctness: the cost to the fringe vertex with minimum cost cannot be further improved.  %The associated entailment is not trivial.
...
one hop from a popped vertex ``\emph{mom}''. Initially this is trivial since $\m{i}=0$, and we restore it as \m{i} increments by updating costs when they can be improved, as in lines~\ref{code:update1}~and~\ref{code:update2}.
\item $\m{inv\_unseen}$ (less than \m{i}): no finite-cost path is known from any popped vertex to $\m{dst}$. As above, this is restored as \m{i} increments.
...
into $\m{inv\_unpopped}$ after considering whether the popping of
\m{u} improves the best-known cost of \m{dst}.
...
Again, as $\m{i}$ increments, we consider whether the popping 
of \m{u} reveals a finite-cost path to~\m{dst}. 
...
(\texttt{i~=~size}), and the PQ and the \m{dist} and \m{prev} arrays
...
all vertices are in \m{inv\_popped}, either optimally reachable or altogether unreachable.
We are done.
