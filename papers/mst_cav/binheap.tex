...
The user calls \texttt{edit\_priority} by supplying the \emph{key} of the item
they wish to modify, and the binary heap uses the key to
locate the item before calling \texttt{sink} or \texttt{swim}. To keep everything linked together, the \texttt{key\_table} is modified during \texttt{exch}ange.
