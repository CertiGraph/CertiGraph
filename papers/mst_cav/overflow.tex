...
%This 
These bounds are optimal: if the input is any less restricted, the postcondition will fail.  Indeed, the last restriction on \texttt{inf} is not meaningful when $\texttt{size} = 1$, so in that special case we further require that any (self-loop) edges cost less than $\texttt{inf}$.
Whenever
