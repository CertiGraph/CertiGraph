...
\subsubsection{Our ``undirected'' definitions.}
...
An undirected path (\texttt{upath}) is list of vertices that are 
pairwise adjacent. Two vertices are \texttt{connected} when a \texttt{upath}
...
A \texttt{simple\_upath} is a \texttt{upath} that has no duplicate vertices. 
An undirected cycle (\texttt{ucycle}) is a \texttt{upath} whose first 
and last vertices are the same.  A \texttt{simple\_ucycle} is a cycle whose
