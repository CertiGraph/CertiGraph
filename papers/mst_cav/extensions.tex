...
CertiGraph library. The following two sections then take advantage of 
...
in problems where efficient label reads are required, but 
manipulations such as sorting edges by weight are not. 
...
While handling these spatial wrinkles, we develop utilities easily that 
...
The property of undirectedness requires no special conditions 
when it comes to spatial representation in separation logic, 
and so we seamlessly share spatial representations and related 
lemmas across directed and undirected graphs.
...
\subsubsection{Edge list representation, multi-graph trimming, and subtle differences of graphs in different representations}
