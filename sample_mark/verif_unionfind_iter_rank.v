...
Proof.
  ...
  } Unfocus. destruct H2. apply false_Cne_eq in HRE. subst ppa. assert (uf_root g x p) by (split; intros; auto; eapply parent_loop; eauto).
  ...
  Focus 2. { destruct H5 as [? [? ?]]. forward. apply (exp_right g'). entailer !. apply (exp_right p). entailer !. rewrite <- (uf_equiv_root_the_same g); auto. } Unfocus.
