...
Proof.
  ...
  1: rewrite H0; simpl; apply (@vertices_at_ramif_1_stable _ _ _ _ SGBA_VST (SGP_VST sh) g (vvalid g) x (r, pa)); auto.
  ...
  - destruct H1. apply true_Cne_neq in HRE.
    ...
    unlocalize [whole_graph sh g].
    1: rewrite H3; simpl; apply (@vertices_at_ramif_1_stable _ _ _ _ SGBA_VST (SGP_VST sh) g (vvalid g) ppa (mr, mgpa)); auto.
  ...
  - destruct H1. apply false_Cne_eq in HRE. subst ppa. assert (uf_root g x p) by (split; intros; auto; apply (parent_loop g p (vlabel g p) y); auto).
    ...
    + destruct H4 as [? ?]. apply true_Cne_neq in HRE. remember (vgamma g' xv) as rpa eqn:?H. destruct rpa as [xr xpa]. symmetry in H6.
      ...
      unlocalize [whole_graph sh g'].
      1: rewrite H6; apply (@vertices_at_ramif_1_stable _ _ _ _ SGBA_VST (SGP_VST sh) g' (vvalid g') xv (xr, xpa)); auto.
