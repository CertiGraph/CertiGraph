...
(* In the encoding, A goes on the left in the encoding *)
...
(* Here, in the encoding, A goes on the right in the encoding *)
Program Definition prodswap_tc {A} {B} (tcB : tagcode B) (tcA : tagcode A) : tagcode (A * B) :=
  let X := prod_tc tcB tcA in
    Build_tagcode (A * B) (tc_bitsize tcA + tc_bitsize tcB) 
                          (fun a => tc_encode X (snd a, fst a)) 
                          (fun i => (snd (tc_decode X i), fst (tc_decode X i))) _ _.
Next Obligation.
  unfold X. rewrite plus_comm. apply tc_encode_range.
Qed.
Next Obligation.
  unfold X. rewrite tc_decode_encode. destruct a. trivial.
Qed.

Program Definition padleft_tc {A} (n : nat) (tcA : tagcode A) : tagcode A :=
  let X := prod_tc (unit_tc n) tcA in
    Build_tagcode A (tc_bitsize X) 
                    (fun a => tc_encode X (tt, a)) 
                    (fun i => snd (tc_decode X i)) _ _.
Next Obligation.
  unfold X. apply tc_encode_range.
Qed.
Next Obligation.
  unfold X. rewrite tc_decode_encode. trivial.
Qed.

Program Definition padright_tc {A} (n : nat) (tcA : tagcode A) : tagcode A :=
  let X := prod_tc tcA (unit_tc n) in
    Build_tagcode A (tc_bitsize X) 
                    (fun a => tc_encode X (a,tt)) 
                    (fun i => fst (tc_decode X i)) _ _.
Next Obligation.
  unfold X. apply tc_encode_range.
Qed.
Next Obligation.
...
Lemma prod_tc_mod: forall A B (tcA : tagcode A) (tcB: tagcode B) a b, 
  tc_encode (prod_tc tcA tcB) (a,b) mod two_power_nat (tc_bitsize tcB) = tc_encode tcB b.
Proof.
  intros.
  simpl.
  generalize (two_power_nat_pos (tc_bitsize tcB)). intro.
  rewrite Zplus_mod, Z_mod_mult, Z.add_0_l, Z.mod_mod, Zmod_small.
  trivial.
  apply tc_encode_range.
  omega.
Qed.

Lemma prod_tc_div: forall A B (tcA : tagcode A) (tcB: tagcode B) a b,
  tc_encode (prod_tc tcA tcB) (a,b) / two_power_nat (tc_bitsize tcB) = tc_encode tcA a.
Proof.
  intros. simpl.
  rewrite Z.add_comm, Z_div_plus_full, Zdiv_small, Z.add_0_l.
  trivial.
  apply tc_encode_range.
  generalize (two_power_nat_pos (tc_bitsize tcB)).
  omega.
Qed.
